...
Computable-Path-Substitution .U V τ σ σ' .Γ Δ _ _ σ∶Γ⇒CΔ σ'∶Γ⇒CΔ τ∶σ∼σ' (ΛR {U} {Γ} {A} {M} {B} Γ,A⊢M∶B) validΔ = 
  ...
  func-EE (lllR (convER (Path-Substitution Γ,A⊢M∶B
    ...
    let σSR : Sub (U , -Term) W
        σSR = x₀:= N' •SR Rep↑ -Term ρ • Sub↑ -Term σ' in
    let σSR∶Γ,A⇒Θ : σSR ∶ Γ ,T A ⇒C Θ
        σSR∶Γ,A⇒Θ = compC (compSRC (botsubC N'∈EΘA) (Rep↑-typed ρ∶Δ⇒Θ)) (Sub↑C σ'∶Γ⇒CΔ) in --REFACTOR Common pattern
    ...
    let step2 : x₀:= N' • Sub↑ -Term (ρ •RS σ') ∼ σSR
    ...
    let sub-rep-comp : ∀ (σ : Sub U V) (N : Term W) → M ⟦ x₀:= N •SR Rep↑ _ ρ • Sub↑ _ σ ⟧ ≡ M ⟦ Sub↑ _ σ ⟧ 〈 Rep↑ _ ρ 〉 ⟦ x₀:= N ⟧
        sub-rep-comp σ N = let open ≡-Reasoning in
             ...
             ∎ in
    let ih : EE Θ (M ⟦ σ₁ ⟧ ≡〈 B 〉 M ⟦ σSR ⟧) 
                  (M ⟦⟦ extendPS (ρ •RP τ) Q ∶ σ₁ ∼ σSR ⟧⟧)
        ih = (Computable-Path-Substitution (U , -Term) W (extendPS (ρ •RP τ) Q) σ₁ σSR (Γ ,T A) Θ _ _ σ₁∶Γ,A⇒Θ σSR∶Γ,A⇒Θ
             (change-ends {σ = x₀:= N' • Sub↑ -Term (ρ •RS σ')} {σ' = σSR} (extendPS-typedC (compRP-typedC {ρ = ρ} {τ} {σ} {σ'} τ∶σ∼σ' ρ∶Δ⇒Θ) 
               Q∈EΘN≡N')
