...
SNmainlemma {P} {Q} {Γ} {σ = σ} {Δ} (Λ {φ = φ} {δ} {ψ} Γ,φ⊢δ∶ψ) hyp = 
  ...
  , (λ R {Θ} ρ ε ρ∶Δ→Θ ε∈CΔφ → NeutralC {Γ = Θ} {δ = appP ((ΛP φ δ) ⟦ σ ⟧ 〈 ρ 〉) ε} {φ = close ψ}
       {!!} 
       (appNeutral ((ΛP φ δ) ⟦ σ ⟧ 〈 ρ 〉) ε)
       {!lemma!}) where
  lemma : ∀ {R} {φ} {δ} {ρ : Rep Q R} {Θ} {ε} χ → 
          ρ ∶ Δ ⇒R Θ →
          C Θ (close φ) ε →
          appP ((ΛP φ δ) ⟦ σ ⟧ 〈 ρ 〉) ε ⇒ χ → C Θ (close ψ) χ
  lemma _ ρ∶Δ→Θ ε∈CΘφ (redex βI) = {!!}
  lemma _ ρ∶Δ→Θ ε∈CΘφ (app λφδε→χ) = {!!}
--TODO "Change type" lemma
\end{code}