...
\text{If $P$ is in normal form, then } \\
...
Note that we have the expected properties: If $E \rightarrow F$ then $E[x:=M] \rightarrow F[x:=M]$ and $E[p:=\delta] \rightarrow F[p:=\delta]$.  However, we do \emph{not} have $E[e:=P] \rightarrow F[e:=P]$ in general.  Counterexample:
$\reff{\lambda x:A.\lambda y:B.z}e \rightarrow
\triplelambda f:y =_B y'. \reff{z}$ but $\reff{\lambda x:A.\lambda y:B.z} \reff{w} \not\twoheadrightarrow \triplelambda f:y =_B y'. \reff{z}$.
