...
\\
\>[0]\AgdaIndent{2}{}\<[2]%
\>[2]\AgdaInductiveConstructor{ΛPR} \AgdaBound{Δ⊢φ∶Ω} \AgdaSymbol{(}\AgdaPostulate{change-type} \AgdaSymbol{(}\AgdaFunction{weakening} \AgdaBound{Γ,φ⊢δ∶ψ} \AgdaSymbol{(}\AgdaInductiveConstructor{ctxPR} \AgdaBound{Δ⊢φ∶Ω}\AgdaSymbol{)} \AgdaSymbol{(}\AgdaFunction{liftRep-typed} \AgdaBound{ρ∶Γ⇒RΔ}\AgdaSymbol{))} \AgdaSymbol{(}\AgdaFunction{liftRep-upRep} \AgdaBound{ψ}\AgdaSymbol{))}\<%
\\
