...
\\
\>\AgdaComment{--open import PHOPL.SN}\<%
\end{code}
}
...
\>\<%
\\
\>\AgdaComment{--Computable-Sub \_ \_ (⊥R \_) validΔ = E-⊥ validΔ}\<%
\end{code}
}
...
\begin{code}%
\>\AgdaComment{--Computable-Sub σ σ∶Γ⇒CΔ (⊃R Γ⊢φ∶Ω Γ⊢ψ∶Ω) validΔ = }\<%
\\
\>\AgdaComment{--  ⊃-E (Computable-Sub σ σ∶Γ⇒CΔ Γ⊢φ∶Ω validΔ) (Computable-Sub σ σ∶Γ⇒CΔ Γ⊢ψ∶Ω validΔ)}\<%
\end{code}
...
\begin{code}%
\>\AgdaComment{-- Computable-Sub σ σ∶Γ⇒CΔ (appPR Γ⊢δ∶φ⊃ψ Γ⊢ε∶φ) validΔ = appP-E }\<%
\\
\>\AgdaComment{--  (Computable-Sub σ σ∶Γ⇒CΔ Γ⊢δ∶φ⊃ψ validΔ) }\<%
\\
\>\AgdaComment{--  (Computable-Sub σ σ∶Γ⇒CΔ Γ⊢ε∶φ validΔ) -\}}\<%
\end{code}
...
\item[1]
\begin{code}%
\>\AgdaComment{--Computable-Sub σ \{Γ = Γ\} \{Δ = Δ\} σ∶Γ⇒CΔ (ΛR \{A = A\} \{M\} \{B\} Γ,A⊢M∶B) validΔ = EI}\<%
\end{code}
Typability is easy to check.

\begin{code}%
\>\AgdaComment{--  (ΛR (substitution Γ,A⊢M∶B (ctxTR validΔ) (liftSub-typed (subC-typed σ∶Γ⇒CΔ)))) (}\<%
\end{code}
...
\begin{code}%
\>\AgdaComment{--  (λ \{W\} Θ \{ρ\} \{N\} ρ∶Δ⇒RΘ Θ⊢N∶A computeN → }\<%
\end{code}

We have that $(\sigma, x:=N) : (\Gamma, x : A) \rightarrow \Theta$ is computable, 
\begin{code}%
\>\AgdaComment{--  let σN∶ΓA⇒CΘ : extendSub (ρ •RS σ) N ∶ Γ ,T A ⇒C Θ}\<%
\\
\>\AgdaComment{--      σN∶ΓA⇒CΘ = extendSubC (compRSC ρ∶Δ⇒RΘ σ∶Γ⇒CΔ) (EI Θ⊢N∶A computeN) in}\<%
\end{code}
and so the induction hypothesis gives
$M[\sigma, x:=N] \in E_\Theta(B)$.
\begin{code}%
\>\AgdaComment{--  let EΘMN : E Θ (ty B) (M ⟦ extendSub (ρ •RS σ) N ⟧)}\<%
\\
\>\AgdaComment{--      EΘMN = Computable-Sub (extendSub (ρ •RS σ) N) σN∶ΓA⇒CΘ Γ,A⊢M∶B (context-validity Θ⊢N∶A) in }\<%
\end{code}
...
\AgdaHide{
\begin{code}%
\>\AgdaComment{\{- Computable-Sub σ σ∶Γ⇒CΔ (ΛPR Γ⊢M∶A Γ⊢M∶A₁) validΔ = \{!!\}\<\\
