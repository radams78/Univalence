...
\\
\>[0]\AgdaIndent{2}{}\<[2]%
\>[2]\AgdaInductiveConstructor{SNI} \AgdaSymbol{\_} \AgdaSymbol{(λ} \AgdaBound{\_} \AgdaSymbol{→} \AgdaFunction{βR-exp'} \AgdaBound{SNφ} \AgdaSymbol{(}\AgdaBound{SNδ} \AgdaSymbol{\_} \AgdaBound{δ⇒δ'}\AgdaSymbol{)} \AgdaBound{SNε} \AgdaSymbol{(}\AgdaFunction{SNred} \AgdaBound{SNδε} \AgdaSymbol{(}\AgdaFunction{apredr} \AgdaFunction{SUB} \AgdaPostulate{R-respects-sub} \AgdaSymbol{(}\AgdaInductiveConstructor{osr-red} \AgdaBound{δ⇒δ'}\AgdaSymbol{))))}\<%
\\
...
\\
\>[2]\AgdaIndent{4}{}\<[4]%
\>[4]\AgdaSymbol{(}\AgdaFunction{SNred} \AgdaBound{SNPMNQ} \AgdaSymbol{(}\AgdaFunction{apredr} \AgdaFunction{SUB} \AgdaPostulate{R-respects-sub} \AgdaSymbol{(}\AgdaInductiveConstructor{osr-red} \AgdaBound{P⇒P'}\AgdaSymbol{))))}\<%
\\
