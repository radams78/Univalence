...
Again, if we reduce one of the subterms, then we can apply the induction hypothesis; otherwise, we must show that
\begin{align*} & ( \left( \lambda q : F =_{A \rightarrow \Omega} G . \triplelambda p : x =_A y. \right. \\
& \qquad \qquad \mathsf{trans}_{A_2 \rightarrow \cdots \rightarrow A_n \rightarrow \Omega} \left( \overline{P} , \right. \\
& \qquad \qquad \left. \trans*{q_{xx} \reff{x}}{Q} \right) \chi \vec{\theta})^+ \alpha \vec{\beta} \in \SN \end{align*} 
where $P[y := x, p := \reff{x}] \twoheadrightarrow \overline{P}$.  It is therefore sufficient to prove that
\begin{align*} & 
& \qquad \qquad (\mathsf{trans}_{A_2 \rightarrow \cdots \rightarrow A_n \rightarrow \Omega} \left( \overline{P}[x:=L_1] , \right. \\
& \qquad \qquad \left. \trans*{\alpha_{L_1 L_1} \reff{L_1}}{Q[x:=L_1, y:=L_1', p:=\theta_1} \right) \theta_2 \cdots \theta_n)^+ \alpha \vec{\beta} \in \SN \end{align*} 
\todo{Finish proof}
\end{proof}
