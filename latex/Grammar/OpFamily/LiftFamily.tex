...
\\
\>[0]\AgdaIndent{2}{}\<[2]%
\>[2]\AgdaKeyword{postulate} \AgdaPostulate{liftOp'''-idOp} \AgdaSymbol{:} \AgdaSymbol{∀} \AgdaSymbol{\{}\AgdaBound{V}\AgdaSymbol{\}} \AgdaBound{A} \AgdaSymbol{→} \<[39]%
\>[39]\<%
\\
\>[2]\AgdaIndent{27}{}\<[27]%
\>[27]\AgdaFunction{liftOp'''} \AgdaBound{A} \AgdaSymbol{(}\AgdaFunction{idOp} \AgdaBound{V}\AgdaSymbol{)} \AgdaFunction{∼op} \AgdaFunction{idOp} \AgdaSymbol{(}\AgdaFunction{dom'} \AgdaBound{V} \AgdaBound{A}\AgdaSymbol{)}\<%
\end{code}
}
