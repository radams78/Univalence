...
\begin{enumerate}
\item \textbf{$\lambda o e$ --- Predicative Higher-Order Propositional Logic} A type theory with:
  \begin{itemize}
  ...
  \end{itemize}
\item \textbf{$\lambda o e$ --- P.H.O.P.L. with Equality}
Make $\delta =_\phi \epsilon$ a proposition.  (So we can form propositions like $M =_A N \epsilon \supset N =_A M$.)
\item \textbf{$\lambda o \forall$} Close $\Omega$ under $\forall$.  (So we can form propositions like $\forall x : \phi. x =_A x$.)
\end{enumerate}
...
\mode<all>{\input{Grammar/Substitution/PreOpFamily.lagda}}
\mode<all>{\input{Grammar/Substitution/OpFamily.lagda}}
