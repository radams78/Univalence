...
β-creates-rep {c = app} (app₂ (app lam (app₂ A (app₂ δ out₂))) (app₂ ε out₂)) {σ = σ} βI = record { 
  created = δ ⟦ x₀:= ε ⟧ ; 
  red-created = βI ; 
...
Rep↑-typed {P} {Q = Q} {ρ = ρ} {φ = φ} ρ∷Γ→Δ zero = let open ≡-Reasoning {A = Expression (Palphabet Q , -Proof) (nonVarKind -Prp)} in 
  {!!}
{-  begin 
    φ 〈 toRep ρ 〉 〈 (λ _ → ↑) 〉
  ≡⟨⟨ rep-comp {E = φ} ⟩⟩
    φ 〈 (λ K x → ↑ (toRep ρ _ x)) 〉      
  ≡⟨ rep-cong {E = φ} (λ x → trans (sym (toRep-↑ {Q} (toRep ρ _ x) )) (toRep-comp {g = suc} {f = ρ} x)) ⟩
    φ 〈 toRep (lift 1 ρ) •R (λ _ → ↑) 〉  
  ≡⟨ rep-comp {E = φ} ⟩
    φ 〈 (λ _ → ↑) 〉 〈 toRep (lift 1 ρ) 〉 
  ∎ -}
...
prop-triv-red {P} {app imp (app₂ φ (app₂ ψ out₂))} (app (appl φ→φ')) = prop-triv-red {P} φ→φ'
prop-triv-red {P} {app imp (app₂ φ (app₂ ψ out₂))} (app (appr (appl ψ→ψ'))) = prop-triv-red {P} ψ→ψ'
...
SR (Λ Γ⊢δ∷φ) (redex ())
SR {P} (Λ Γ⊢δ∷φ) (app (appl φ→φ')) with prop-triv-red φ→φ'
... | ()
SR (Λ Γ⊢δ∷φ) (app (appr (appl (Λ δ→δ')))) = Λ (SR Γ⊢δ∷φ δ→δ')
...
C-typed {Γ = Γ} {φ = app imp (app₂ φ (app₂ ψ out₂))} {δ = δ} = λ x → subst (λ P → Γ ⊢ δ ∷ P) --{x = φ 〈 _ 〉 ⇒ ψ 〈 _ 〉} {y = φ 〈 _ 〉 ⇒ ψ 〈 _ 〉} 
...
C-rep {P} {Q} {Γ} {Δ} {app imp (app₂ φ (app₂ ψ out₂))} {δ} {ρ} (Γ⊢δ∷φ⇒ψ , Cδ) ρ∷Γ→Δ = (subst 
  --{i = zero} {A = Expression (Palphabet Q) (nonVarKind -Prp)} 
  (λ x → Δ ⊢ δ 〈 toRep ρ 〉 ∷ x) 
  --{a = φ 〈 _ 〉 〈 toRep ρ 〉 ⇒ ψ 〈 _ 〉 〈 toRep ρ 〉} {b = (φ ⇒ ψ) 〈 _ 〉}
  (cong₂ _⇛_ 
...
C-red {Γ = Γ} {φ = app imp (app₂ φ (app₂ ψ out₂))} {δ = δ} (Γ⊢δ∷φ⇒ψ , Cδ) δ→δ' = (SR (subst (λ x → Γ ⊢ δ ∷ x) 
  (cong₂ _⇛_ (rep-cong {E = φ} (λ ())) (rep-cong {E = ψ} (λ ()))) 
  Γ⊢δ∷φ⇒ψ) δ→δ') , 
...
NeutralC-lm _ _ hyp2 .(app app (app₂ _ (app₂ _ out₂))) (app (appr (appl ε→ε'))) = hyp2 _ ε→ε'
...
mutual
  ...
  NeutralC {P} {Γ} {δ} {app imp (app₂ φ (app₂ ψ out₂))} Γ⊢δ∷φ→ψ neutralδ hyp = (subst (λ P₁ → Γ ⊢ δ ∷ P₁) (rep-cong {E = φ ⇛ ψ} (λ ())) Γ⊢δ∷φ→ψ) , 
