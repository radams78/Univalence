...
\begin{code}
  Reduction : Set₁
  Reduction = ∀ {V} {K} {C : Kind (-Constructor K)} → Constructor C → Subexpression V (-Constructor K) C → Subexpression' V K → Set

  data osr' (R : Reduction) : ∀ {V} {C} {K} → Subexpression V C K → Subexpression V C K → Set where
    redex : ∀ {V} {K} {C : Kind (-Constructor K)} {c : Constructor C} {EE : Subexpression V (-Constructor K) C} {F : Subexpression' V K} → R c EE F → osr' R (app c EE) F
    ...
    out : ∀ {V} {K} {M N : Subexpression' V K} → osr' R M N → osr' R (out M) (out N)
    Λ : ∀ {V} {K} {L} {M N : Subexpression (V , K) -Abstraction L} → osr' R M N → osr' R (Λ M) (Λ N)

  mutual
    data osr (R : Reduction) : ∀ {V} {K} → Subexpression' V K → Subexpression' V K → Set where
      redex : ∀ {V} {K} {C : Kind (-Constructor K)} {c : Constructor C} {EE : Subexpression V (-Constructor K) C} {F : Subexpression' V K} → R c EE F → osr R (app c EE) F
      app : ∀ {V} {K} {C : Kind (-Constructor K)} {c : Constructor C} {MM NN : Subexpression V (-Constructor K) C} → osrB R MM NN → osr R (app c MM) (app c NN)

    data osrB (R : Reduction) : ∀ {V} {K} {C : Kind (-Constructor K)} → Subexpression V (-Constructor K) C → Subexpression V (-Constructor K) C → Set where
      appl : ∀ {V} {K} {L} {C : Kind (-Constructor L)} {M N : Subexpression V -Abstraction K} {PP : Subexpression V (-Constructor L) C} → osrA R M N → osrB R (app₂ M PP) (app₂ N PP)
      appr : ∀ {V} {K} {L} {C : Kind (-Constructor L)} {M : Subexpression V -Abstraction K} {NN PP : Subexpression V (-Constructor L) C} → osrB R NN PP → osrB R (app₂ M NN) (app₂ M PP)

    data osrA (R : Reduction) : ∀ {V} {K} → Subexpression V -Abstraction K → Subexpression V -Abstraction K → Set where
      out : ∀ {V} {K} {M N : Subexpression' V K} → osr R M N → osrA R (out M) (out N)
      Λ : ∀ {V} {K} {L} {M N : Subexpression (V , K) -Abstraction L} → osrA R M N → osrA R (Λ M) (Λ N)

  _→〈_〉_ : ∀ {V} {K} → Subexpression' V K → Reduction → Subexpression' V K → Set
  M →〈 R 〉 N = osr' R M N

  data red (R : Reduction) {V} {K} : Subexpression' V K → Subexpression' V K → Set where
    osr-red : ∀ {M} {N} → M →〈 R 〉 N → red R M N
    ref : ∀ {M} → red R M M
    trans-red : ∀ {M N P : Subexpression' V K} → red R M N → red R N P → red R M P

  _↠〈_〉_ : ∀ {V} {K} → Subexpression' V K → Reduction → Subexpression' V K → Set
  M ↠〈 R 〉 N = red R M N

  data redB (R : Reduction) {V} {K} {C : Kind (-Constructor K)} : Subexpression V (-Constructor K) C → Subexpression V (-Constructor K) C → Set where
    osr-red : ∀ {M} {N} → osr' R M N → redB R M N
    ref : ∀ {M} → redB R M M
    trans-red : ∀ {M N P : Subexpression V (-Constructor K) C} → redB R M N → redB R N P → redB R M P

  redapp : ∀ {R : Reduction} {V} {K} {C : Kind (-Constructor K)} (c : Constructor C) {MM NN : Subexpression V (-Constructor K) C} →
  ...
  data redA (R : Reduction) {V} {K}: Subexpression V -Abstraction K → Subexpression V -Abstraction K → Set where
    osr-red : ∀ {A} {B} → osr' R A B → redA R A B
    ref : ∀ {A} → redA R A A
    trans-red : ∀ {A B C : Subexpression V -Abstraction K} → redA R A B → redA R B C → redA R A C
  ...
  redout : ∀ {R : Reduction} {V} {K} {E F : Subexpression' V K} → red R E F → redA R (out E) (out F)
  redout (osr-red E→F) = osr-red (out E→F)
  redout ref = ref
  redout (trans-red E↠F F↠G) = trans-red (redout E↠F) (redout F↠G)

  redΛ : ∀ {R : Reduction} {V} {K} {L} {A B : Subexpression (V , K) -Abstraction L} → redA R A B → redA R (Λ A) (Λ B)
  redΛ (osr-red A→B) = osr-red (Λ A→B)
  redΛ ref = ref
  redΛ (trans-red A↠B B↠C) = trans-red (redΛ A↠B) (redΛ B↠C)

  data conv (R : Reduction) {V} {K} : Subexpression' V K → Subexpression' V K → Set where
...
\begin{code}
  ...
  respect-rep R = ∀ {U} {V} {K} {C} {c} {EE} {F : Subexpression' U K} {ρ : Rep U V} → R {U} {K} {C} c EE F → R c (rep EE ρ) (rep F ρ)

  record create-rep (R : Reduction) : Set where
    field
      created : ∀ {U} {V} {K} {C} {c} {EE : Subexpression U (-Constructor K) C} {F : Subexpression' V K} {ρ : Rep U V} → R {V} {K} {C} c (rep EE ρ) F → Subexpression' U K
      red-created : ∀ {U} {V} {K} {C} {c} {EE} {F : Subexpression' V K} {ρ : Rep U V} (δ : R {V} {K} {C} c (rep EE ρ) F) → R c EE (created δ)
      rep-created : ∀ {U} {V} {K} {C} {c} {EE} {F : Subexpression' V K} {ρ : Rep U V} (δ : R {V} {K} {C} c (rep EE ρ) F) → rep (created δ) ρ ≡ F

  respect-sub : Reduction → Set
  respect-sub R = ∀ {U} {V} {K} {C} {c} {EE} {F : Subexpression' U K} {ρ : Sub U V} → R {U} {K} {C} c EE F → R c (EE ⟦ ρ ⟧B) (F ⟦ ρ ⟧)

  mutual
    reposr : ∀ {R : Reduction} → respect-rep R →
      ∀ {U} {V} {K} {M N : Subexpression' U K} {ρ : Rep U V} → M →〈 R 〉 N → rep M ρ →〈 R 〉 rep N ρ
  ...
  mutual
    subosr : ∀ {R : Reduction} → respect-sub R →
      ∀ {U} {V} {K} {M N : Subexpression' U K} {ρ : Sub U V} → M →〈 R 〉 N → M ⟦ ρ ⟧ →〈 R 〉 N ⟦ ρ ⟧
    subosr hyp (redex M▷N) = redex (hyp M▷N)
    subosr hyp (app MM→NN) = app (subosrB hyp MM→NN)

    subosrB : ∀ {R : Reduction} → respect-sub R →
      ∀ {U} {V} {K} {C : Kind (-Constructor K)} {M N : Subexpression U (-Constructor K) C} {ρ : Sub U V} → osr' R M N → osr' R (M ⟦ ρ ⟧B) (N ⟦ ρ ⟧B)
    subosrB hyp (appl M→N) = appl (subosrA hyp M→N)
    subosrB hyp (appr NN→PP) = appr (subosrB hyp NN→PP)

    subosrA : ∀ {R : Reduction} → respect-sub R →
      ∀ {U} {V} {K} {A B : Subexpression U -Abstraction K} {ρ : Sub U V} → osr' R A B → osr' R (A ⟦ ρ ⟧A) (B ⟦ ρ ⟧A)
...
\begin{code}
  ...
  mutual
    subredr : ∀ {U} {V} {K} {ρ σ : Sub U V} {R : Reduction} {E : Subexpression' U K} → respect-rep R →
      ρ ↠〈 R 〉s σ → E ⟦ ρ ⟧ ↠〈 R 〉 E ⟦ σ ⟧
    subredr {E = var x} _ ρ↠σ = ρ↠σ _ x
    subredr {E = app c _} hyp ρ↠σ = redapp c (subredrB hyp ρ↠σ)

    subredrB : ∀ {U} {V} {K} {C : Kind (-Constructor K)} {ρ σ : Sub U V} {R : Reduction} {EE : Subexpression U (-Constructor K) C} →
      respect-rep R →
    ...
    subredrA : ∀ {U} {V} {K} {ρ σ : Sub U V} {R : Reduction} {A : Subexpression U -Abstraction K} →
      respect-rep R →
...
\begin{code}
  data SN (R : Reduction) {V} {K} : Subexpression' V K → Set where
    SNI : ∀ E → (∀ F → E →〈 R 〉 F → SN R F) → SN R E

  data SNA (R : Reduction) {V} {K} : Subexpression V -Abstraction K → Set where
    SNI : ∀ A → (∀ B → osr' R A B → SNA R B) → SNA R A

  data SNB (R : Reduction) {V} {K} {C : Kind (-Constructor K)} : Subexpression V (-Constructor K) C → Set where
...
\begin{code}
  SNsubexp : ∀ {V} {K} {C : Kind (-Constructor K)} {c : Constructor C} {EE : Subexpression V (-Constructor K) C} {R : Reduction} → SN R (app c EE) → SNB R EE
  ...
  SNoutA : ∀ {V} {K} {R : Reduction} {E : Subexpression' V K} → SNA R (out E) → SN R E
  SNoutA {E = E} (SNI .(out E) SNE) = SNI E (λ F E→F → SNoutA (SNE (out F) (out E→F)))
  
  SNlam : ∀ {V} {K} {L} {R : Reduction} {A : Subexpression (V , K) -Abstraction L} → SNA R (Λ A) → SNA R A
  SNlam {A = A} (SNI .(Λ A) SNlamA) = SNI A (λ B A→B → SNlam (SNlamA (Λ B) (Λ A→B)))

  SNsub : ∀ {U} {V} {K} {E : Subexpression' U K} {σ : Sub U V} {R : Reduction} → 
  ...
  SNred : ∀ {V} {K} {E F : Subexpression' V K} {R : Reduction} → SN R E → E ↠〈 R 〉 F → SN R F
  SNred {V} {K} {E} {F} (SNI .E SNE) (osr-red E→F) = SNE F E→F
  SNred SNE ref = SNE
  SNred SNE (trans-red E↠F F↠G) = SNred (SNred SNE E↠F) F↠G

  SNrep : ∀ {U} {V} {K} {ρ : Rep U V} {E : Subexpression' U K} {R : Reduction} → create-rep R → SN R E → SN R (rep E ρ)
  SNrep {U} {V} {K} {ρ} {E} {R} hyp (SNI .E SNE) = SNI (rep E ρ) (λ F E〈ρ〉→F → 
    let E₀ : Subexpression' U K
  ...
  SNrep' : ∀ {U} {V} {K} {ρ : Rep U V} {E : Subexpression' U K} {R : Reduction} → respect-rep R → SN R (rep E ρ) → SN R E
