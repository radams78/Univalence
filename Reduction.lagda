...
\begin{code}
  data _⇒_ : Relation where
    ...
    appl : ∀ {V K A C M N PP} → M ⇒ N → app₂ {V} {K} {A} {C} M PP ⇒ app₂ N PP
    appr : ∀ {V K A C M NN PP} → NN ⇒ PP → app₂ {V} {K} {A} {C} M NN ⇒ app₂ M PP
...
--TODO Make lettering consistent for subexpressions
  redappl : ∀ {V K A C M N PP} → M ↠ N → app₂ {V} {K} {A} {C} M PP ↠ app₂ N PP
  redappl (osr-red A→B) = osr-red (appl A→B)
  redappl ref = ref
  redappl (trans-red A↠B B↠C) = trans-red (redappl A↠B) (redappl B↠C)

  redappr : ∀ {V K A C M NN PP} → NN ↠ PP → app₂ {V} {K} {A} {C} M NN ↠ app₂ M PP
...
\begin{code}
    ...
    liftOp-red hyp ρ↠σ K (Taxonomy.↑ x) = subst₂ _↠_ (sym (liftOp-↑ x)) (sym (liftOp-↑ x)) (respects-red hyp (ρ↠σ K x))
...
--TODO Extract common pattern
    ...
    apredl {E = app₂ {A = A} E F} hyp ρ↠σ = trans-red (redappl (apredl {E = E} hyp (liftOp'-red {A = A} hyp ρ↠σ))) (redappr (apredl {E = F} hyp ρ↠σ))
\end{code}
...
\begin{code}
  ...
  create-osr _ out₂ ()
  create-osr hyp (app₂ E F) {σ = σ} (appl σE⇒E') =     
    let open Respects-Creates.creation (create-osr hyp E σE⇒E') in 
    record { 
      created = app₂ created F; 
      red-created = appl red-created; 
      ap-created = cong (λ x → app₂ x (F 〈 σ 〉)) ap-created
      }
  create-osr hyp (ToGrammar.app₂ {A = A} E F) {σ = σ} (appr {PP = F'} σF⇒F') =     
    ...
    record { 
      ...
      ap-created = cong (app₂ (E 〈 OpFamily.liftOp' replacement A σ 〉)) ap-created
      }
\end{code}
...
\begin{code}
  ...
  SNsubbodyl : ∀ {V K A C M N} → SN (app₂ {V} {K} {A} {C} M N) → SN M
  SNsubbodyl {V} {K} {A} {C} {M} {N} (SNI .(app₂ M N) SNM) = SNI M (λ M' M⇒M' → SNsubbodyl (SNM (app₂ M' N) (appl M⇒M')))

  SNsubbodyr : ∀ {V K A C M N} → SN (app₂ {V} {K} {A} {C} M N) → SN N
  SNsubbodyr {V} {K} {A} {C} {M} {N} (SNI .(app₂ M N) SNN) = SNI N (λ N' N⇒N' → SNsubbodyr (SNN (app₂ M N') (appr N⇒N')))
--TODO Extract common pattern
