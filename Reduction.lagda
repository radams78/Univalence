...
data _⇒_ : Relation where
  redex : ∀ {V K} {C : Kind (-Constructor K)} {c : Constructor C} 
    {E : Subexpression V (-Constructor K) C} {F : Expression V K} → 
    R c E F → app c E ⇒ F
  app : ∀ {V K} {C : Kind (-Constructor K)} {c : Constructor C} 
    {E F : Subexpression V (-Constructor K) C} → E ⇒ F → app c E ⇒ app c F
  appl : ∀ {V K A L C E E' F} → 
    E ⇒ E' → _,,_ {V} {K} {A} {L} {C} E F ⇒ E' ,, F
  appr : ∀ {V K A L C E F F'} → 
