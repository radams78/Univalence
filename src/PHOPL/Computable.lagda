...
& \quad \forall (\Delta \supseteq \Gamma) (\delta \in E_\Delta(M)). \\
& \qquad (M \{\})^+ \delta \in E_\Delta(M) \text{ and } (M\{\})^- \delta \in
E_\Delta(M) \} \\
...
\begin{lm}
If $t \in E_\Gamma(T)$ then $t \in \SN$.
\end{lm}
...
If $A \equiv \Omega$: we have $\Gamma \vdash \reff{M}_{N N} \reff{N} \vec{P} : L =_A L'$, hence $\Gamma \vdash \reff{MN} \vec{P} : L =_A L'$
using Generation.  Also, $\reff{M}_{N N} \reff{N} \vec{P} \in \SN$ hence $\reff{MN} \vec{P} \in \SN$ by Lemma \ref{lm:SNred1}.
...
$\Gamma \vdash \reff{(\lambda x:A.M)N \vec{N}}_{L_1 L_1'} \vec{P} : L =_{\Omega} L'$ using
Generation.  Lemma \ref{lm:SN}.\ref{lm:SN2} gives that $\reff{(\lambda x:A.M) N \vec{N}}_{L_1 L_1'} \vec{P} \in \SN$.
...
Suppose $\Gamma, x : A \vdash M : B_1 \rightarrow \cdots \rightarrow B_n \rightarrow C$.  Let $N \in E_\Gamma(A)$ and $N_i \in E_\Gamma(B_i)$ for $i = 1, \ldots, n$.  Suppose
$N\{\} \in E_\Gamma(N =_A N)$ and $N_i\{\} \in E_\Gamma(N_i =_{B_i} N_i)$ for all $i$.
If
...
This will follow from the previous lemma (part 2) once we have shown
\[ M \{ x:= N \{ \} : N \sim N \} \in E_\Gamma(M[x:=N] = M[x:=N]) \]
\end{proof}
