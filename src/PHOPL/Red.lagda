...
There are two reduction relations associated with $\lambda o e$.  The usual $\beta$-reduction relation on terms is the one used for the typing rules.  We also define a second relation on terms, proofs and paths.  Our goal in this paper is to prove strong normalization for typed terms with respect to this second relation.

\paragraph{$\beta$-Reduction}
Let \emph{$\beta$-contraction} $\rhd_\beta$ be the relation between expressions consisting of all pairs of the form
\[ (\lambda x:A.M)N \rhd_\beta M[x:=N] \enspace . \]
Let $\beta$-conversion $\simeq_\beta$ be the congruence generated by $\rhd_\beta$.

\paragraph{Reduction Relation}
\todo{Better name}
...
\item
As explained above, these relations do not play any part in the rules of deduction of $\lambda o e$; 
the relation $\simeq_\beta$ that appears in the rules of $\lambda o e$ is different.
In particular, we \emph{do} allow $\beta$-contraction of open terms there: thus, the judgement $x : \Omega, p : (\lambda y : \Omega . y) x \vdash p : x$ is derivable in $\lambda o e$.  We shall always use the subscript $\beta$ when referring to this relation.
\end{enumerate}
