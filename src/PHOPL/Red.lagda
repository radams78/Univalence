...
\begin{proof}
A straightforward induction in each case.  We give the details for one case in part 3, where $M \equiv (\lambda y:A.L)L'$ and $M' \equiv L[y:=L']$.  
Remembering that $M$ and $M'$ must be closed terms, we have
\begin{align*}
\lefteqn{((\lambda y:A.L)L') \{ \}} \\
 & \eqdef (\triplelambda e : y =_A y' . L \{ y := e : y \sim y' \})_{L' L'} L'\{ \} \\
& \rightarrow L \{ y := e : y \sim y' \} [ y := L' , y' := L', e := L' \{ \} ] & (\text{Lemma \ref{lm:subpathsub}.\ref{lm:subpathsubii}}) \\
& \equiv L \{ y := L' \{ \} : L' \sim L' \} \\
& \equiv L [ y := L' ] \{ \} & (\text{Lemma \ref{lm:subpathsub}.\ref{lm:subpathsubi}})
\end{align*}
\end{proof}
