...
The rule (ref) uses Lemma \ref{lm:Eref}, and the rule ($\supset^*$) uses Lemma \ref{lm:Esupset}.  The rule (univ) uses Lemma \ref{lm:Euniv}.
...
\Gamma \vdash N : A \rightarrow B
     \end{array}} \]
This is proven similarly to the previous case, using Lemma \ref{lm:wte5}.2.
\end{proof}
