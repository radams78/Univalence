...
data _⇒_ {V} : ∀ {K} {C} → 
  ...
  redex : ∀ {K} {C : Kind -Constructor} {c : Constructor (C ⟶ K)} 
    {E : Subexpression V -Constructor C} {F : Expression V K} → 
    R c E F → app c E ⇒ F
  app : ∀ {K} {C : Kind -Constructor} {c : Constructor (C ⟶ K)} 
    {E F : Subexpression V -Constructor C} → 
    E ⇒ F → app c E ⇒ app c F
  appl : ∀ {A L E E' F} → 
    E ⇒ E' → _,,_ {V} {A} {L} E F ⇒ E' ,, F
  appr : ∀ {A L E F F'} → 
    F ⇒ F' → _,,_ {V} {A} {L} E F ⇒ E ,, F'
...
\begin{code}
  ...
  postulate liftOp''-red : ∀ {U V A} {ρ σ : Op U V} → respects' → 
                           ρ ↠s σ → liftOp'' A ρ ↠s liftOp'' A σ
