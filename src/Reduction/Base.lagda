...
data _⇒_ {V} : ∀ {K} {C} → Subexp V K C → Subexp V K C → Set where
  redex : ∀ {K} {C : Kind -ListAbs} {c : Constructor (SK C K)} 
    {E : Subexp V -ListAbs C} {F : Expression V K} → 
    R c E F → app c E ⇒ F
  app : ∀ {K} {C : Kind -ListAbs} {c : Constructor (SK C K)} 
    {E F : Subexp V -ListAbs C} → 
    E ⇒ F → app c E ⇒ app c F
  appl : ∀ {A AA E E' F} → 
...
data _↠_ {V C K} (E : Subexp V C K) : 
  Subexp V C K → Set where
  osr-red : ∀ {F} → E ⇒ F → E ↠ F
  ref : E ↠ E
  trans-red : ∀ {F G} → E ↠ F → F ↠ G → E ↠ G
...
app-red : ∀ {V} {AA} {K} {c : Constructor (SK AA K)} {EE FF : ListAbs V AA} → EE ↠ FF → app c EE ↠ app c FF
app-red (osr-red EE⇒FF) = osr-red (app EE⇒FF)
app-red ref = ref
app-red (trans-red EE↠FF FF↠GG) = trans-red (app-red EE↠FF) (app-red FF↠GG)

∷-redl : ∀ {V} {AA} {A} {E E' : Abs V A} {FF : ListAbs V AA} → E ↠ E' → (_∷_ {V} {A} {AA} E FF) ↠ (E' ∷ FF)
∷-redl (osr-red E⇒E') = osr-red (appl E⇒E')
∷-redl ref = ref
∷-redl (trans-red E↠E' E'↠E'') = trans-red (∷-redl E↠E') (∷-redl E'↠E'')

∷-redr : ∀ {V} {AA} {A} {E : Abs V A} {FF FF' : ListAbs V AA} → FF ↠ FF' → (_∷_ {V} {A} {AA} E FF) ↠ (E ∷ FF')
∷-redr (osr-red FF⇒FF') = osr-red (appr FF⇒FF')
∷-redr ref = ref
∷-redr (trans-red FF↠FF' FF'↠FF'') = trans-red (∷-redr FF↠FF') (∷-redr FF'↠FF'')

∷-red : ∀ {V} {AA} {A} {E E' : Abs V A} {FF FF' : ListAbs V AA} → E ↠ E' → FF ↠ FF' → (_∷_ {V} {A} {AA} E FF) ↠ (E' ∷ FF')
∷-red E↠E' FF↠FF' = trans-red (∷-redl E↠E') (∷-redr FF↠FF')
